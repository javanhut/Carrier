...
use crate::runtime::namespaces::{NamespaceManager, NamespaceConfig};
use crate::runtime::cgroups::{CgroupManager, CgroupConfig};
use crate::runtime::security::{SecurityManager, SecurityConfig};
use crate::runtime::process::{ContainerProcess, ProcessConfig};
use crate::runtime::network::{NetworkManager, NetworkConfig, NetworkMode};
...
impl Container {
    ...
    fn validate_config(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if !cmd_path.exists() {
            ...
            if !found {
                return Err(format!("Command not found in rootfs: {}", self.config.command[0]).into());
            }
        }
